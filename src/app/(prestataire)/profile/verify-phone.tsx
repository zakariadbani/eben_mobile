...
export default function VerifyChangedPartnerPhoneScreen(): React.ReactElement {
  ...
  const verify = async (valid: boolean, code: string) => {
    ...
    try {
      ...
      router.replace('/(prestataire)/profile/edit');
    } catch {
      setIsValid(false);
    ...
    return (
      <Screen whatsapp={false}>
        <View flex alignItems="center" justifyContent="center" gap={16} style={styles.empty}>
          ...
          <Button title={t('partner.editProfile.title')} onPress={() => router.replace('/(prestataire)/profile/edit')} />
        </View>
      </Screen>
    );
  }
