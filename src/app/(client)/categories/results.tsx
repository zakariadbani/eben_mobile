...
const CategoryResultsScreen: React.FC = () => {
  ...
  const renderItem = ({ item }: { item: ResultItem }) => {
    // Occasion no longer branches into a draft-add UI here — the L3 occasion
    // leaf routes straight to the Brands step now (D2); this screen only
    // ever shows priced products (en_stock, or a direct/legacy occasion
    // product query) with the normal price/wishlist card.
    return (
